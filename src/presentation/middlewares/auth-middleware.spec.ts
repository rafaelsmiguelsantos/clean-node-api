...
import { LoadAccountByTokenSpy } from '../test/mock-add-account'
import { forbidden, HttpRequest, ok, serverError } from './index'
...
describe('Auth Middleware', () => {
  ...
  test('Should return 500 if LoadAccountByToken throws', async () => {
    const { sut, loadAccountByTokenSpy } = makeSut()
    jest.spyOn(loadAccountByTokenSpy, 'load').mockImplementationOnce(throwNewError)
    const httpResponse = await sut.handle(mockRequest())
